...
//Gets the non-teaching handled by the user
$query="SELECT ot_id,des FROM non_teaching WHERE signatant = '$id'";
$subjects = mysql_query($query);

//Checks if user handles any non-teaching
if (mysql_num_rows($subjects)==0)
echo "<p>You do not handle any Non-teaching</p>";


while ($sub = mysql_fetch_row($subjects))
{
    
 //Gets the sections of the students who are involved in the non-teaching handled by the user    
$query  = "SELECT secid,section FROM sections WHERE secid IN (SELECT secid FROM st_sec WHERE stid IN (SELECT stid FROM st_non_teaching WHERE st_non_teaching.ot_id = $sub[0]))";
...
if($total!=0)
{
        ...
        while ($row = mysql_fetch_row($sections))
        {
                ...
                echo '<td><a href ="sig_sect_non.php?'.  create_parameters($array).'">'.$row[1].'</td></tr>';
                $count2++;
        }
        echo '</table>';
}
