...
//Gets the current announcement of the selected subject
$result = mysql_query("SELECT announce FROM subjects WHERE sub_id = '$title'") or die ("Error in query: $query." . mysql_error());
...
if(isset($_POST['submit']))
{
        ...
        mysql_query("UPDATE subjects SET announce ='".mysql_escape_string($_POST['comment'])."',post_date = '$date[0]' WHERE sub_id = '$title'") or die('Error');
        header("location:ann_sub.php");
}
        echo '<div class="select">'.'<form action="sub_comment.php?sub='.$title.'" method="post">';
        echo '<input type="radio" name="sub" value="'.$title.'" hidden="true" checked>';
