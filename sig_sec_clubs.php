...
//Gets the members of the selected club in the selected section
$result= mysql_query("SELECT students.stid,lname,fname,cleared,clubs.clid FROM students JOIN st_clubs USING(stid) WHERE st_clubs.clid =$sub AND students.stid IN (SELECT stid FROM st_sec WHERE secid= $sec) ORDER BY lname") or die("ERROR:". mysql_error());
$count2=0;
...
while ($row = mysql_fetch_row($result))
{
        ...
        echo '<td><a href="st_comments.php?subid='.$row[4]."&stid=$row[0]&type=1".'">Comment</a></td>';
        
if($row[3]=="CLEARED")
            echo '<script language="Javascript">document.getElementById("check'.$count2.'").innerHTML = " ";</script>';
        $count2++;
}
echo '<tr class="button"><td></td><td></td><td></td><td></td><td><input class="delete" type="submit" name="clubs" value="Clear"></td></tr></form></table>';
$clr->footer();
?>