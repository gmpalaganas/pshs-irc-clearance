...
$trail->add('Section', $_SERVER['PHP_SELF'], 2);
$trail->output();
...
//Gets the students of the selected subject in the selected section
$result= mysql_query("SELECT students.stid,lname,fname,cleared,st_subjects.sub_id FROM students, st_subjects WHERE st_subjects.sub_id = $sub AND students.stid IN (SELECT stid FROM st_sec WHERE secid= $sec)  AND st_subjects.stid = students.stid ORDER BY lname");
$count2=0;
while ($row = mysql_fetch_row($result))
{
        ...
        echo '<td><a href="st_comments.php?subid='.$row[4]."&stid=$row[0]&type=0".'">Comment</a></td>';
            if($row[3]=="CLEARED")
            echo '<script language="Javascript">document.getElementById("check'.$count2.'").innerHTML = " ";</script>';
        $count2++;
}
echo '<tr class="button"><td></td><td></td><td></td><td></td><td><input class="delete" type="submit" name="subjects" value="Clear"></td></tr></form></table>';
